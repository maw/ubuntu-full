...
Personal, disposable Docker environment for one-off testing, builds, and
experimentation. Hopefully useful, but not meant for distribution or
production work.
...
- `rust.sh`, `uv.sh` — vestigial installer scripts (Rust and uv are now copied
  directly in the Dockerfile)
- `ubuntu-full.sh` — old launcher script, superseded by `uf` + compose

## Building and running

`uf` invokes `docker compose run` against `compose.yaml`, resolving the
compose file relative to the script so it works from any directory. The
caller's `$PWD` is mounted as `/work`.
...
Compose tags the image as `ubuntu-full:latest` automatically; no manual
tagging needed. Named volumes (`ubuntu-full-home`, `ubuntu-full-rustup`,
`ubuntu-full-cargo`) are created on first run.
